...
export let checkbox = {
  iniciar: () => {
    ...
    document.getElementsByTagName('body')[0].addEventListener('change', evento => {
      if (evento.target.tagName.toLowerCase() === 'input' && evento.target.getAttribute('type').toLowerCase() === 'checkbox') {
...
export class ListaSelect {
    ...
    this.elementoUl.addEventListener('click', evento => {
      this.antesDeCambiar().then(cambiar => {
        ...
        if (clicado.tagName.toLowerCase() === 'li') {
          let li; let lis = this.elementoUl.querySelectorAll('li')
  ...
  seleccionar (indice) {
    let i = 0; let li; let lis = this.elementoUl.querySelectorAll('li')
    for (li of lis) {
      if (i === indice) {
  ...
  agregar (clave, valor) {
    ...
    let li; let lis = this.elementoUl.getElementsByTagName('li')
    for (li of lis) {
