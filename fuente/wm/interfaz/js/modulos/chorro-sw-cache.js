...
export default function chorrear (callback) {
  ...
  if (chorroIds !== '') { // Si hay algún id en la lista
    let url = `${urlApiPropia}/apis/chorro/v1/?chorro=${chorroIds}`
    let datosDeRedRecibidos = false
    let idioma = obtenerIdiomaUrl()
    // Pide datos en la red
    window.fetch(url, { method: 'get', headers: { 'Accept-Language': idioma } })
      .then(respuesta => {
    ...
    if (window.caches) {
      window.caches.match(url).then(resCache => {
        if (resCache) {
