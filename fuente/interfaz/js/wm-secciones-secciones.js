import estilos from '../estilo/wm-secciones-secciones.scss';
...
function hacer() {
	...
	document.getElementById('guardar').addEventListener('click', () => {
		...
		fetch( url, { method: metodo, headers: reqHeaders, body: JSON.stringify( seccion ) } )
		.then( respuesta => respuesta.json() )
		.then( resSecc => {
			if ( resSecc.error ) {
				...
				return fetch( comun.urlBaseApi + resSecc.url, { method: 'get', headers: reqHeaders } );
			}
		})
		.then( respuesta => respuesta.json() )
		.then( seccion => {
