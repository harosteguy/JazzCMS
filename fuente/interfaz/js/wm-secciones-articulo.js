import estilos from '../estilo/wm-secciones-articulo.scss';
...
function hacer() {
	...
	let headerAuth = 'Basic ' + btoa( localStorage.getItem('uid') + ':' + localStorage.getItem('token') ),
		reqHeaders = { 'Authorization': headerAuth, 'Accept-Language': idioma },
		urlApi = `${comun.urlBaseApi}/apis/wm-articulus/v1/blogs/`;
	...
	.then( editor => {
		...
		// Obtiene blogs
		return fetch( urlApi, { method: 'get', headers: reqHeaders } )
	})
	.then( respuesta => respuesta.json() )
	.then( listaBlogs => {
		if ( listaBlogs.length > 0 ) {
			var opt, selBlog = document.getElementById('secciones');
	...
	function mostrarArticulo ( blogBase, artiBase ) {
		...
		let articulo;
		fetch( urlApi + blogBase + '/articulos/' + artiBase, { method: 'get', headers: reqHeaders } )
		.then( respuesta => respuesta.json() )
		.then( arti => {
			// Actualiza URL
			history.pushState( null, null, `/${idiomaUrl}wm/secciones/articulo/${blogBase}/${artiBase}`);
			articulo = arti;		// Para usar el el siguiente .then
			// Blog del artículo
			...
			document.getElementById('publicado').checked = arti.publicado == 1 ? true : false;
	...
	document.getElementById('secciones').addEventListener('change', () => {
		...
		fetch( urlApi + blogBase + '/categorias/', { method: 'get', headers: reqHeaders } )
		.then( respuesta => respuesta.json() )
		.then( categorias => {
	...
	function limpiarEditor() {
		history.pushState( null, null, `/${idiomaUrl}wm/secciones/articulo/`);		// Actualiza URL
		...
		document.querySelector('.imgPrincipal .foto').src = '/interfaz/img/sin_foto.jpg';
	...
	function guardarArticulo() {
		...
		fetch( url, { method: metodo, headers: reqHeaders, body: JSON.stringify( arti ) } )
		.then( respuesta => respuesta.json() )
		.then( resArti => {
			if ( resArti.error ) {
				comun.esperaAjax( false, 'guardarArti');
