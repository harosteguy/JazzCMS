import estilos from '../estilo/wm-secciones-indice.scss';
...
function hacer() {
	...
	.then( categorias => {
		...
		if ( localStorage.getItem('esAdmin') === '1' ) {
			return fetch(`${comun.urlBaseApi}/apis/wm-articulus/v1/autores?blogBase=${blogBase}`, { method: 'get', headers: reqHeaders } );
		}
	...
	function listaOnClick( claveValor ) {
		...
		fetch(`${comun.urlBaseApi}/apis/wm-articulus/v1/blogs/${claveValor.clave}/categorias`, { method: 'get', headers: reqHeaders } )
		.then( respuesta => respuesta.json() )
		.then( categorias => {
			let selCat = document.getElementById('filtroCategoria');
			...
			// Autores para el admin
			if ( localStorage.getItem('esAdmin') === '1' ) {
				return fetch(`${comun.urlBaseApi}/apis/wm-articulus/v1/autores?blogBase=${claveValor.clave}`, { method: 'get', headers: reqHeaders } );
			}
		})
		.then( respuesta => respuesta.json() )
		.then( autores => {
	...
	function listarArticulos ( url ) {
		return new Promise( ( resuelve, rechaza ) => {
			...
			.then( artis => {
				...
				resuelve( artis.articulos.length );
			})
			.catch( error => {
				comun.esperaAjax( false, 'listarArtis' );
				rechaza( error );
